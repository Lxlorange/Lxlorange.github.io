...
利用优化算法迭代多次，可得最优解 $\alpha^* = (\alpha_1^*, \alpha_2^*, \dots, \alpha_N^*)^T$。


根据KKT条件，$w^* = \sum_{i=1}^N \alpha_i^* y_i x_i$且可以选择 $\alpha^*$ 的一个正分量 $\alpha_j^* > 0$（这个 $j$ 对应的 $x_j$ 就是一个支持向量），利用互补松弛性，有$y_j(w^* \cdot x_j + b^*) - 1 = 0$，代入：

$$
y_j \left( \left( \sum_{i=1}^N \alpha_i^* y_i x_i \right) \cdot x_j + b^* \right) - 1 = 0
$$

利用 $y_j^2 = 1$，两边同乘以 $y_j$，解得 $b^*$：
